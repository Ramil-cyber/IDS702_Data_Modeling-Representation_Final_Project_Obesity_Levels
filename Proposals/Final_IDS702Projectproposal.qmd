...
obesity_data |> 
  ...
  labs(
    title = "BMI vs Physical Activity Frequency", 
...
Genetic predisposition or shared lifestyle habits could play a role in the higher BMI values observed in the `yes` group.
...
obesity_data |>ggplot(aes(x = NObeyesdad)) + geom_bar(alpha = 0.7) +labs(
  ...
  y = "Number of people") +theme_minimal() + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
obesity_data |> 
  ...
  facet_wrap(~ CALC) +
  labs(
    title = "Distribution of Meal Frequency by Obesity Levels",
    subtitle = "Faceted by Alcohol Consumption",
    x = "Number of Meals per Day (NCP)", 
    y = "Number of people", 
    fill = "Obesity Level"
  ) +
  theme_minimal()
...
The plot highlights the dominant pattern of individuals having 3 meals per day across all groups. Alcohol consumption seems to show some correlation with obesity levels, particularly for those in the `Sometimes` group. However, data is comparatively thin for the `Always` and `Frequently` groups.
...
-   **Variables**: 14 variables and 6,497 observations of Portuguese "Vinho Verde" wine with 11 continuous variables (e.g., acidity, pH level), 2 categorical variables (red vs white, quality-low, medium, high) and 1 interval variable (quality score which ranges from 1 to 10)
...
-   (Interaction Term) ***`volatile acidity`*** \* ***`total sulfur dioxide`***
...
ggplot(wine_data, aes(x = alcohol, y = citric_acid)) +
  geom_point(alpha = 0.5, size = 0.2, color = "purple") + facet_wrap(~ quality_category) +
...
```{r, fig.width=6, fig.height=2.5}
plot1 <- ggplot(wine_data, aes(x = factor(quality), y = total_sulfur_dioxide)) +
  geom_boxplot() + labs(x = "Quality Score", y = "Total Sulfur Dioxide") + theme_minimal()
plot2 <- ggplot(wine_data, aes(x = factor(quality), y = volatile_acidity)) + geom_boxplot() +
  labs(x = "Quality Score", y = "Volatile Acidity") + theme_minimal()
plot_grid(plot1, plot2, ncol = 2, rel_widths = c(1, 1))

ggplot(wine_data, aes(x = volatile_acidity, y = total_sulfur_dioxide, color = quality)) +
  geom_point(alpha = 0.5, size = 0.2) +
  scale_color_gradient(low = "white", high = "purple") +
  labs(x = "Volatile Acidity", y = "Total Sulfur Dioxide", color = "Quality Score") +
